 ...
 * knowledge proofs.
 * @dev This code was vendored from
...
abstract contract GovernorCountingFractional is Governor {
  ...
  /// @notice Thrown when params data submitted with a vote does not match the convention expected for fractional
  /// voting.
  ...
  function _countVoteNominal(uint256 proposalId, address account, uint128 totalWeight, uint8 support) internal {
    if (_proposalVotersWeightCast[proposalId][account] != 0) revert GovernorCountingFractional__VoteWeightExceeded();
