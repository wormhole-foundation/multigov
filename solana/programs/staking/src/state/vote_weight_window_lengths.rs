...
pub fn push_new_window_length<'info>(
    ...
    // Step 1: Immutable borrow to get latest_index and latest_window_length
    let (current_index, latest_window_length) = {
        let vote_weight_window_length = vote_weight_window_length_loader.load()?;
        if vote_weight_window_length.next_index == 0 {
            (0, None) // if next_index is 0, set to None
        } else {
            let latest_index = vote_weight_window_length.next_index - 1;
            ...
            (
                ...
                Some(latest_window_length),
            )
        }
    };

    if let Some(ref latest_window_length) = latest_window_length {
        if latest_window_length.timestamp != current_timestamp {
