...
import { getProgramTransactions, printTransactionEvents } from "../utils/parse_transactions";
...
//   await printTransactionEvents(connection, transactionSignature);
  ...
  try {
    const transactions = await getProgramTransactions(connection, programId, limit);
